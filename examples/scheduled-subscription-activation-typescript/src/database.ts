...
export async function isolatedDatabase() {
  ...
  if (!connection) throw new Error('Set DATABASE_URL to your local PostgreSQL development connection.');
  ...
  return {connectionString: url.toString(), async close() {
    try { if (!/^subscrio_blog_[a-f0-9]{32}$/.test(name)) throw new Error('Unexpected database name');
